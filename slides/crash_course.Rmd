...
### 7. Los modelos complejos requieren de un esfuerzo titánico para programárse (¿alguna vez han programado un verosimilitud?)
...
--
  ...
  c. Sintaxis SQL para manipulación de datos tabulares `dplyr`
---
# Entonces... ¿R o Python? 
### 3 Razones para usar R y no Python
...
--
  ...
  7. Versionamiento semántico y colaboración con Git
...
Utilizar '`=`'  ó  '`<-`'  para asignar variables es lo mismo
...
Sin embargo, no puedes declarar los argumentos de una función con `<-`
...
Digamos que `=` es un caracter reservado para argumentos en funciones
...
A pesar de ser equivalentes es buena práctica asignar objetos a variables con `<-`
...
Creamos vectores con la función `c()` con cada elemento separado por una ' , '. (OJO también es posible crear vectores vacios)
...
### En computación existe algo llamado la *tercera forma normal de Codd*, tiene que ver con bases de datos relacionadas, pero en términos estadísticos significa:
...
Una de las razones por la que R es tan poderoso para hacer análisis de datos es su **sintaxis**

--

El primer caso importante (de hecho en python se incorporó a pandas) es el **operador pipa** ` %>% `de *magrittr* (no olvides que su atajo de teclado es CTRL/CMND + SHIFT + M)
...
### Otro ejemplo de lo poderosa que es esta sintaxis es el *subseting pipe*
...
--
  ...
  4. Revisa el contenido del objeto con la función `glimpse()` (recuerda usar la sintaxis avanzada)
